    ...
    def Evaluate(self):
        ...
        for review_id in self.dict_True_n_Extracted_Features.keys():
            ...
            if self.Evaluation_Config.value == 2:
                true_features = [w for w in true_features if len(
                    w.split()) >= 2 and len(w.split()) <= 4]

            self.true_aspects_list.extend(true_features)

            tps, fps, fns, revise_extracted_feature_info = self.Token_Based_Subset_Matching(
            ...
            self.predicted_list.extend(predicted_features)
        ...
        try:
            ...
            recall_safe = 0
        try:
            fscore_safe = 2*((precision_safe*recall_safe) /
                             (precision_safe+recall_safe))
        except ZeroDivisionError as err:
            fscore_safe = 0

        evaluation_token_partial = {'precision': '%.3f' % precision_safe,
                                    'recall': '%.3f' % recall_safe, 'fscore': '%.3f' % fscore_safe}
        print('Precision : %.3f , Recall , %.3f , F1-score : %.3f' %
              (precision_safe, recall_safe, fscore_safe))
        ...
        for pindex, pred_feature in enumerate(predicted_features_review):
            ...
            for t_index, true_feature in enumerate(app_true_features):
                ...
                if len(pred_feature_words) == len(true_feature_words):
                    set_match = all(
                        [true_word in pred_feature_words for true_word in true_feature_words])
                else:
                    set_match = False
            ...
            if found == True:
                tps = tps + 1
                review_extracted_features[pindex] = (
                    pred_feature[0], pred_feature[1], 'TP')
                ...
                for t_index, true_feature in enumerate(app_true_features):
                    ...
                    if len(pred_feature_words) != len(true_feature_words):
                        if len(pred_feature_words) > len(true_feature_words):
                            ...
                            set_match = all(
                                [pred_word in true_feature_words for pred_word in pred_feature_words])
                        else:
                            set_match = False
                    else:
                        set_match = False
                ...
                if found == True:
                    ...
                    review_extracted_features[pindex] = (
                        pred_feature[0], pred_feature[1], 'FP')
