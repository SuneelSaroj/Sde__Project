...
import spacy
import Patterns_used
from Preprocessing import preprocessing
import Model_Evaluation
import ReadData
from ReadData import XML_REVIEW_DATASET
...
importlib.reload(Patterns_used)
...
class Model:
    ...
    def PreprocessData(self):
        ...
        for review_id in self.data.keys():
            ...
            textProcessor = preprocessing(self.appName, review_sent_text)
            unclean_sents = textProcessor.SegmemtintoSentences(sents_segmented)
            review_clean_sentences = textProcessor.GetCleanSentences()
            Patterns_used_Obj = Patterns_used.Patterns_used(
                self.appName, review_id, review_clean_sentences, unclean_sents)
            sents_with_features = Patterns_used_Obj.ExtractFeatures_Analyzing_Sent_POSPatterns()

            for sid in sents_with_features.keys():
                reviewSent_wise_features.extend(
                    sents_with_features[sid]['extracted_features'])
    ...
    def CleanFeatures(self, true_features_dict):
        ...
        for review_id in true_features_dict.keys():
            ...
            for findex, feature_info in enumerate(lst_extracted_feature):
                ...
                duplicate_words = all(stemmer.stem(
                    x) == stemmer.stem(words[0]) for x in words)
                ...
                contain_app_words = all(
                    w in lemma_app_words for w in app_words)
            ...
            for feature_term1 in list_clean_feaures:
                status = any([feature_term1[0] in f[0]
                             for f in list_clean_feaures if f[0] != feature_term1[0]])
                if status == True:
                    clean_features_list.remove(feature_term1)

            true_features_dict[review_id]['predicted_features'] = list(
                set(clean_features_list))
...
def ReadAppDescWithAspectTerms(app_data):
    ...
    sents_with_aspect_terms[app_name] = {'review_sent': app_description, 'true_features': true_features, 'predicted_features': [
    ], 'review-id': app_name}  # ,'sent_class': sent_class}
...
if __name__ == '__main__':
    ...
    if extraction_mode == EXTRACTION_MODE.USER_REVIEWS:

        # user review data for the evaluation
        user_review_dataset = DATASETS.SHAH_UNION
        ...
        if user_review_dataset.name not in [DATASETS.GUZMAN_PLUS.name, DATASETS.LAPTOP.name, DATASETS.RESTAURANT.name]:

            for app in ENGLISH_APPS:
                print('*' * 5, app.name, '*' * 5)
                objXML_DS = XML_REVIEW_DATASET(user_review_dataset, app)
                reviewSents_with_true_features = objXML_DS.ReadReview()

                obj_safe = Model(
                    app.name, reviewSents_with_true_features, extraction_mode, nlp)
                ...
                obj_Evaluation = Evaluate(
                    app, dict_true_features, extracted_features, evaluation_mode)
            ...
            print('Precision : %.3f , Recall , %.3f , F1-score : %.3f' %
                  (np.average(lst_precision), np.average(lst_recall), np.average(lst_fscore)))
            print(
                '----------------------------------------------------------------------------------------------')
        else:
            print('*' * 5, user_review_dataset.name, '*' * 5)

            objXML_DS = XML_REVIEW_DATASET(
                user_review_dataset, user_review_dataset)
            reviewSents_with_true_features = objXML_DS.ReadReview()

            obj_safe = Model(user_review_dataset.name,
                             reviewSents_with_true_features, extraction_mode, nlp)
            true_features_dict, extracted_features = obj_safe.GetReviewsWithExtractedFeatures()
            dict_true_features = obj_safe.CleanFeatures(true_features_dict)
            obj_Evaluation = Evaluate(
                user_review_dataset.name, dict_true_features, extracted_features, evaluation_mode)
            dict_features_evaluated, token_eval_results = obj_Evaluation.PerformEvaluation()

            print(
                '----------------------------------------------------------------------------------------------')
    elif extraction_mode == EXTRACTION_MODE.APP_DESCRIPTION:
        data_path = '../Datasets/APP_DESCRIPTION/app_descriptions_with_manual_feature_extraction.json'
        with open(data_path) as json_data:
            ...
            for app_data in data:
                ...
                AppDesc_with_true_features = ReadAppDescWithAspectTerms(
                    app_data)
                obj_safe = Model(
                    app_data['app_name'], AppDesc_with_true_features, extraction_mode, nlp)
                true_features_dict, extracted_features = obj_safe.GetReviewsWithExtractedFeatures()
                # print(extracted_features)
                dict_true_features = obj_safe.CleanFeatures(true_features_dict)
                # print(dict_true_features)
                obj_Evaluation = Evaluate(
                    app_data['app_name'], dict_true_features, extracted_features, evaluation_mode)
                dict_features_evaluated, token_eval_results = obj_Evaluation.PerformEvaluation()
